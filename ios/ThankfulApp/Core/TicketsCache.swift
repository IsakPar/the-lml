...
struct CachedTicket: Codable, Equatable {
  let jti: String
  let token: String
  let orderId: String
...
final class TicketsCacheService {
  ...
  func save(orgId: String, tickets: [CachedTicket]) {
    if let data = try? JSONEncoder().encode(tickets) { _ = KeychainHelper.save(key: key(orgId: orgId), data: data) }
  }

  func upsert(orgId: String, ticket: CachedTicket) {
    var arr = list(orgId: orgId)
    if let idx = arr.firstIndex(where: { $0.jti == ticket.jti }) {
      arr[idx] = ticket
    } else {
      arr.append(ticket)
    }
